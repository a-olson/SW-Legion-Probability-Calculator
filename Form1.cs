...
namespace LegionCalculator
{
    public partial class Form1 : Form
    {
         ...
         * READ-ME
         * a-olson
         * Added support for Observation tokens
         * see lines 65, 86, 116, 137, 372-405
         * 13 Dec 2020
         */
        public Form1()
        {
        ...
        void GetInfo()//Gets all the input data
        {
            ...
            iObvTokens = (int)UpDnObvToken.Value;
        ...
        private void btnSimulate_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i <= iItterations; i++)
            {
                ...
                //Spend Observation Tokens
                for(int obv_count = 0; obv_count < iObvTokens; obv_count++)
                 {
                    int RerolledDice = 0;
                    foreach (Dice AttackDie in ListOfAttackDice)
                     {
                         if (RerolledDice < iRerollsObv && AttackDie.ReadResult() == "Blank" && AttackDie.ReadColour() == "RedAttack")
                         {
                             Dice Die = AttackDie;
                             RollAndConvertAttack(ref Die, ref CriticalUsed, ref SurgeTokensUsed);
                             RerolledDice++;
                    ...
                    foreach (Dice AttackDie in ListOfAttackDice)
                     {
                         if (RerolledDice < iRerollsObv && AttackDie.ReadResult() == "Blank" && AttackDie.ReadColour() == "BlackAttack")
                         {
                             Dice Die = AttackDie;
                             RollAndConvertAttack(ref Die, ref CriticalUsed, ref SurgeTokensUsed);
                             RerolledDice++;
                    ...
                    foreach (Dice AttackDie in ListOfAttackDice)
                     {
                         if (RerolledDice < iRerollsObv && AttackDie.ReadResult() == "Blank" && AttackDie.ReadColour() == "WhiteAttack")
                         {
                             Dice Die = AttackDie;
                             RollAndConvertAttack(ref Die, ref CriticalUsed, ref SurgeTokensUsed);
                             RerolledDice++;
